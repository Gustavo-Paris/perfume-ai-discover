-- Corrigir as funções para usar o nome correto da coluna: target_margin_percentage
...
-- Recriar a função calculate_product_total_cost com nome correto da coluna
...
BEGIN
  -- Buscar margem do perfume (coluna correta: target_margin_percentage)
  ...
  RETURN QUERY SELECT 
    ...
    -- CORREÇÃO: Usar multiplicador direto
...
-- Recriar a função update_perfume_margin com nome correto da coluna
...
DECLARE
  ...
  price_result record;
BEGIN
  -- Atualizar margem do perfume (coluna correta: target_margin_percentage)
  UPDATE perfumes 
  SET 
    target_margin_percentage = new_margin_percentage,
    updated_at = now()
  WHERE id = perfume_uuid;
